...
const STUDENT_NAV_ITEMS = [
  ...
  { href: "/skill-insights", label: "Skill Insights", icon: "target" },
...
function Icon({ name }: { name: string }) {
  ...
  switch (name) {
    case "home":
      return (<svg className={common} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 11.5 12 4l8 7.5" strokeLinecap="round" strokeLinejoin="round" /><path d="M6 10v9a1 1 0 0 0 1 1h3v-5h4v5h3a1 1 0 0 0 1-1v-9" strokeLinecap="round" strokeLinejoin="round" /></svg>);
    case "book":
      return (<svg className={common} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 5.5C4 4.7 4.7 4 5.5 4H12v16H5.5A1.5 1.5 0 0 1 4 18.5v-13Z" strokeLinejoin="round" /><path d="M20 5.5c0-.8-.7-1.5-1.5-1.5H12v16h6.5a1.5 1.5 0 0 0 1.5-1.5v-13Z" strokeLinejoin="round" /></svg>);
    case "target":
      return (<svg className={common} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="12" r="8" /><circle cx="12" cy="12" r="4" /><circle cx="12" cy="12" r="0.5" fill="currentColor" /></svg>);
    case "award":
      return (<svg className={common} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="9" r="5" /><path d="M8.5 13.5 7 21l5-2.5L17 21l-1.5-7.5" strokeLinecap="round" strokeLinejoin="round" /></svg>);
    case "calendar":
...
export async function DashboardSidebar({ activeHref, isStaff = false, showInstitutes = false }: { activeHref: string; isStaff?: boolean; showInstitutes?: boolean }) {
  ...
  return (
    <aside className="hidden md:flex flex-col justify-between w-52 shrink-0 py-7 px-4">
      <div>
        <Link href="/" className="flex items-center gap-2.5 mb-9 px-1"><Logo size={30} /><span className="text-[15px] font-bold text-ink tracking-tight">Skill Note</span></Link>
        <nav className="space-y-1">
          {navItems.map((item) => {
            const isActive = item.href === activeHref;
            return <Link key={item.href} href={item.href} className={`flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${isActive ? "bg-cobalt text-white font-medium" : "text-ink-soft hover:bg-bg-warm hover:text-ink"}`}><span className="flex items-center gap-3"><Icon name={item.icon} />{item.label}</span>{item.icon === "bell" && unreadCount > 0 && <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${isActive ? "bg-white/20" : "bg-butter/40 text-ink"}`}>{unreadCount}</span>}</Link>;
          })}
        </nav>
      </div>
      <div>
