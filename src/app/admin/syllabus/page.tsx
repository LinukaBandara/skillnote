  ...
  return (
    <AppShell activeHref="/admin" isStaff showInstitutes>
      <div className="max-w-4xl px-6 md:px-8 py-10 space-y-10">
          ...
          <div className="mt-5 border-t border-rule">
            {(versions ?? []).map((version) => (
              <div key={version.id} className="py-4 border-b border-rule flex items-center justify-between gap-4">
                <div><p className="font-medium text-sm">{version.name}</p><p className="text-xs text-ink-soft mt-1">{version.code} · {version.academic_year_from ?? ""}{version.academic_year_to ? `–${version.academic_year_to}` : ""}</p></div>
        ...
        <section>
          <h2 className="text-base font-medium mb-2">Competencies</h2>
          <p className="text-sm text-ink-soft mb-4">Attach verified competencies to a specific syllabus version and subject.</p>
          <form action={createCompetency} className="border border-rule rounded-xl p-5 grid gap-4 md:grid-cols-2">
            ...
            <input name="code" required placeholder="Competency code" className="field" />
            <input name="title" required placeholder="Competency title" className="field" />
            <textarea name="description" placeholder="Description (optional)" className="field md:col-span-2" rows={2} />
            <input name="position" type="number" min="1" defaultValue="1" placeholder="Position" className="field" />
            <button className="btn-primary px-5 py-2.5 rounded-full text-sm font-medium">Add competency</button>
          </form>
        ...
        <section>
          <h2 className="text-base font-medium mb-2">Competency levels</h2>
          <form action={createCompetencyLevel} className="border border-rule rounded-xl p-5 grid gap-4 md:grid-cols-2">
            ...
            <input name="syllabus_version_id" required placeholder="Syllabus version ID" className="field" />
            <input name="subject_id" required placeholder="Subject ID" className="field" />
            <input name="code" required placeholder="Level code" className="field" />
            <input name="title" required placeholder="Level title" className="field" />
            <textarea name="description" placeholder="Description (optional)" className="field md:col-span-2" rows={2} />
            <input name="position" type="number" min="1" defaultValue="1" placeholder="Position" className="field" />
            <button className="btn-primary px-5 py-2.5 rounded-full text-sm font-medium">Add competency level</button>
          </form>
        </section>
        ...
        <section>
          ...
          <form action={createSubtopic} className="border border-rule rounded-xl p-5 grid gap-4">
            <select name="topic_id" required className="field"><option value="">Select topic</option>{(topics ?? []).map((topic) => { const unit = Array.isArray(topic.syllabus_units) ? topic.syllabus_units[0] : topic.syllabus_units; const subject = unit && (Array.isArray(unit.subjects) ? unit.subjects[0] : unit.subjects); return <option key={topic.id} value={topic.id}>{subject?.name ? `${subject.name} — ` : ""}{topic.title}</option>; })}</select>
        ...
        <section>
          ...
          <form action={createLearningOutcome} className="border border-rule rounded-xl p-5 grid gap-4">
            <select name="subtopic_id" required className="field"><option value="">Select subtopic</option>{(subtopics ?? []).map((subtopic) => { const topic = Array.isArray(subtopic.syllabus_topics) ? subtopic.syllabus_topics[0] : subtopic.syllabus_topics; return <option key={subtopic.id} value={subtopic.id}>{topic?.title ? `${topic.title} — ` : ""}{subtopic.title}</option>; })}</select>
