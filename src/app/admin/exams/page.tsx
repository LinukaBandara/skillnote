            ...
            {(sections ?? []).map((section) => (
              <div key={section.id} className="border border-rule rounded-xl p-5 mb-5">
                ...
                <div className="space-y-2 mb-5">
                  {(section.paper_questions ?? []).map((pq: { id: string; question_number: string | null; marks: number | null; questions: { question_text: string; question_type: string; difficulty: string } | { question_text: string; question_type: string; difficulty: string }[] | null }) => <div key={pq.id} className="text-sm border-t border-rule pt-2"><span className="font-medium">{pq.question_number}.</span> {Array.isArray(pq.questions) ? pq.questions[0]?.question_text : pq.questions?.question_text} <span className="text-xs text-ink-faint">· {Array.isArray(pq.questions) ? pq.questions[0]?.question_type : pq.questions?.question_type} · {pq.marks ?? ""} marks</span></div>)}
                </div>
