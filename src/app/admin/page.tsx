    ...
    <AppShell activeHref="/admin" isStaff showInstitutes={profile.role !== "teacher"}>
      <div className="max-w-xl px-6 md:px-8 py-10">
        ...
        <ul className="border-t border-rule">
          {profile.role === "teacher" && <li className="border-b border-rule"><Link href="/teacher" className="flex items-center justify-between py-5 group"><div><h2 className="text-base font-medium group-hover:text-cobalt transition-colors">Teacher workspace</h2><p className="text-sm text-ink-soft mt-1">Classes, assigned courses and teaching workload</p></div><span className="text-sm text-ink-faint group-hover:text-cobalt">Open</span></Link></li>}
          ...
          {profile.role === "platform_admin" && <li className="border-b border-rule"><Link href="/admin/audit" className="flex items-center justify-between py-5 group"><div><h2 className="text-base font-medium group-hover:text-cobalt transition-colors">Audit log</h2><p className="text-sm text-ink-soft mt-1">Recent platform and staff operations</p></div><span className="text-sm text-ink-faint group-hover:text-cobalt">View</span></Link></li>}
        </ul>
      </div>
    </AppShell>
  );
